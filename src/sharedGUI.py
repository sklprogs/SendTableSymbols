...
from shared import log, Message, WriteTextFile, Text, h_os, Search, Words
...
class SearchBox:
	...
	def __init__(self,obj):
		...
		self.h_entry.close()
		self.h_words = self.obj.h_words # Save time, do not put this to reset
		if self.h_words:
			self._text = self.h_words._text # Do not get text from the widget - it's not packed yet
			self.h_sel = Selection(self.obj,self.h_words)
		else:
			self.Success = False
			Message(func='SearchBox.__init__',type=lev_warn,message=globs['mes'].wrong_input2,Silent=True)
	...
	def h_search(self):
		if self.Success:
			if not hasattr(self,'_h_search'):
				self.reset()
				self._h_search = Search(self._text,self.search())
			return self._h_search
		else:
			log.append('SearchBox.h_search',lev_warn,globs['mes'].canceled)
	...
	def new(self,*args):
		if self.Success:
			...
			log.append('SearchBox.new',lev_warn,globs['mes'].canceled)
...
class Selection:
	...
	def set(self):
		...
		self.h_widget.widget.tag_config('tag',background=self._bg)
...
class ParallelTexts: # Requires Search
	...
	def __init__(self,parent_obj,h_words1,h_words2,h_words3=None,h_words4=None):
		...
		self.txt1 = TextBox(self.frame1,Composite=True,side='left',h_words=self.h_words1)
		self.txt2 = TextBox(self.frame1,Composite=True,side='right',h_words=self.h_words2)
		if self.Extended:
			self.txt3 = TextBox(self.frame2,Composite=True,side='left',h_words=self.h_words3)
			self.txt4 = TextBox(self.frame2,Composite=True,side='right',h_words=self.h_words4)
